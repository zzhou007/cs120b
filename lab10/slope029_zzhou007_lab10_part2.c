 ...
 * Lab section 22
 * slope029_zzhou007_lab10_part2.c
 *
...
int main(void)
{
	...
	TimerOn();
	unsigned short i;
	unsigned char arr[] = "CS120B is Legend... wait for it DARY! CS120B is Legend... wait for it DARY!";
	//run";
	//run
	while(1){
	    int pos;
		...
		for(pos = -1; pos <= 72; pos++ ){ //print array over one
		    int temp = pos;
		    for(cursor = 0; cursor < 16; ++cursor) { //print one line
		        LCD_Cursor(cursor);
				LCD_WriteData(arr[pos]);
		        pos++;
		    }
			while(!TimerFlag){}
			TimerFlag = 0;
